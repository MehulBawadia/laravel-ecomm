...
@section('pageTitle')
    <div class="flex items-end">
        ...
        <h1 class="text-2xl font-Rubik font-bold uppercase tracking-wide leading-none ml-3">Brands</h1>
    </div>
@endsection
    ...
    <section class="latestOrders mt-8 pt-16 px-6">
        <div class="container mx-auto">
            <div class="flex justify-between items-center">
                <h2 class="text-xl font-medium tracking-wide">All Brands</h2>

                <div>
                    <button class="linkAddNewBrand bg-indigo-500 text-sm text-gray-50 py-2 px-3 rounded focus:outline-none hover:bg-indigo-600 focus:bg-indigo-600 tracking-wider font-medium"><i class="fas fa-plus text-xs"></i> Add New</button>
                </div>
            </div>

            @include('admin.brands._create')

            <div class="bg-white rounded-md shadow mt-3 overflow-x-auto brandsTable">
...
@section('pageScripts')
    <script>
        ...
        $('.linkAddNewBrand').on('click', function (e) {
            e.preventDefault();

            $('#sectionAddNewBrand').slideDown().removeClass('hidden').addClass('transition ease-in-out duration-150');
        ...
        $('.linkCancel').on('click', function(e) {
            ...
            $('#sectionAddNewBrand').slideUp().addClass('hidden').removeClass('transition ease-in-out duration-150');
