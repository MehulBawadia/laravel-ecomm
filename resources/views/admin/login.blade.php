...
@section('content')
    <section class="loginAdministrator">
        <div class="container mx-auto">
            <div class="text-center text-indigo-700 my-8 font-bold text-2xl">
                <span class="text-2xl"><i class="fas fa-shopping-cart"></i></span>
                {{ config('app.name') }}
            </div>

            <div class="px-4">
                <h1 class="font-Roboto font-semibold text-2xl uppercase text-gray-900 text-center">Login Administrator</h1>
                    ...
                    <div class="w-full bg-white md:w-4/5 lg:w-1/2 rounded overflow-hidden shadow shadow-zinc-700/50">
                        <form action="{{ route('admin.login.check') }}" method="POST" id="formLoginAdmin">
                            ...
                            <div class="bg-gray-100 py-3 px-6">
                                <button type="submit" class="w-full sm:w-48 bg-indigo-600 text-gray-50 py-2 px-3 rounded focus:outline-none hover:bg-indigo-700 focus:bg-indigo-700 tracking-wider font-medium btnLoginAdmin">Login</button>
